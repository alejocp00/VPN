...
def insert_user_ip_range(userId, ipRangeId):
    conn = sqlite3.connect("vpn.db")
    c = conn.cursor()
    c.execute("INSERT INTO users_iprange VALUES (?, ?)", (userId, ipRangeId))
    conn.commit()
    conn.close()
...
def delete_userIprange(userId, ipRangeId):
    ...
    c = conn.cursor()
    c.execute(
        "DELETE FROM users_iprange WHERE userId=? AND iprangeId=?", (userId, ipRangeId)
    )
    conn.commit()
    conn.close()
