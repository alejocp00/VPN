...
def insert_vlan_ip_range(vlanId, ipRangeId):
    conn = sqlite3.connect("vpn.db")
    c = conn.cursor()
    c.execute("INSERT INTO vlans_iprange VALUES (?, ?)", (vlanId, ipRangeId))
    conn.commit()
    conn.close()
...
def delete_vlanIprange(vlanId, ipRangeId):
    ...
    c = conn.cursor()
    c.execute(
        "DELETE FROM vlans_iprange WHERE vlanId=? AND iprangeId=?", (vlanId, ipRangeId)
    )
    conn.commit()
    conn.close()
